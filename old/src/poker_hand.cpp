...
void PokerHand::score_hand() {
    if(score_ < 0) {
        ...
        if(has_royal_flush(deck_)) {
            //std::cout << "has ace: " << has_ace(deck_) << std::endl;
            //std::cout << "has straight: " << has_straight(deck_) << std::endl;
            //std::cout << "has flush: " << has_straight(deck_) << std::endl;
    ...
    if(score_ == other.score_) {
        ...
        if(show_score() == "FOUR OF A KIND") {
            ...
            for(int i = deck_.size() - 1; i >= 0; i--) {
                ...
                if(other.deck_[i].get_rank() != other.special_value_1_) this_val = deck_[i].get_rank();
            }
            return false;
        }
        ...
        if(show_score() == "PAIR") {
            ...
            std::sort(std::begin(this_vals), std::end(this_vals));
            std::sort(std::begin(other_vals), std::end(other_vals));
            for(int i = this_vals.size() - 1; i >= 0; i--) {
